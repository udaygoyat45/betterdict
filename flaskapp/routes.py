        ...
        try:
            ...
            if current_user.is_authenticated:
                looking_word = Word.query.filter_by(
                    user_id=current_user.id, word=input_word
                ).first()
                if looking_word != None:
                    ...
                    looking_word.level = (
                        1
                        if looking_word.level == 1 or looking_word.level == 0
                        else looking_word.level // 2
                    )
                    ...
                    word = Word(
                        ...
                        due_date=new_time(datetime.utcnow(), 1),
                        level=1,
                    )
                    db.session.add(word)
                    db.session.commit()
                ...
                flash(
                    "Your words will not be saved. Login or Sign Up to save words",
                    "info",
                )
...
def login():
    ...
    return render_template(
        "login.html", name="login", form=form, background_url=blue_gradient()
    )
...
def account():
    ...
    return render_template(
        ...
        name="login",
        form=form,
        background_url="rgb(255, 255, 255)",
        user=user,
    )
    ...
    if request.method == "POST":
        ...
        for key, value in inpjsdata.items():
            current_word = Word.query.filter_by(
                user_id=current_user.id, word=key
            ).first()
